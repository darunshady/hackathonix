...
export default function PaymentModal({ isOpen, onClose, onSave }) {
  /* ── Form state ──────────────────────────────── */
  // Seller-to-customer payment toggle
  const [direction, setDirection] = useState("customer-to-seller"); // default: customer pays seller
  ...
  const handleSave = async () => {
    ...
    const isSellerToCustomer = direction === "seller-to-customer";
    setSaving(true);
    ...
    try {
      ...
      await db.ledger.add({
        ...
        customerId,
        invoiceId: invoiceId || null,
        type: isSellerToCustomer ? "credit" : "debit",
        amount: Number(amount),
        source: "payment",
        synced: 0,
        createdAt: now,
      ...
      if (invoiceId) {
        ...
        if (inv) {
          let newPaid, newBalance, newStatus;
          if (isSellerToCustomer) {
            // Seller pays customer: reduce amountPaid
            newPaid = Math.max(0, (inv.amountPaid || 0) - Number(amount));
            newBalance = (inv.amount ?? 0) - newPaid;
          } else {
            // Customer pays seller: increase amountPaid
            newPaid = (inv.amountPaid || 0) + Number(amount);
            newBalance = Math.max(0, (inv.amount ?? 0) - newPaid);
          }
      ...
      // 3b. Update cached customer balance (amountOwed and sellerDebt)
      const cust = await db.customers.get(customerId);
      if (cust) {
        let newOwed = cust.amountOwed || 0;
        let newSellerDebt = cust.sellerDebt || 0;
        if (isSellerToCustomer) {
          // Seller pays customer: reduce sellerDebt
          newSellerDebt = Math.max(0, newSellerDebt - Number(amount));
        } else {
          // Customer pays seller: reduce amountOwed
          newOwed = Math.max(0, newOwed - Number(amount));
        }
        await db.customers.update(customerId, { amountOwed: newOwed, sellerDebt: newSellerDebt, synced: 0 });
      }
                ...
                <div className="px-6 py-5 space-y-4 max-h-[65vh] overflow-y-auto">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="flex rounded-xl border border-gray-200 overflow-hidden">
                      <button
                        type="button"
                        onClick={() => setDirection("customer-to-seller")}
                        className={`flex-1 py-2.5 text-sm font-medium transition-colors ${
                          direction === "customer-to-seller"
                      ...
                      <button
                        type="button"
                        onClick={() => setDirection("seller-to-customer")}
                        className={`flex-1 py-2.5 text-sm font-medium transition-colors ${
                          direction === "seller-to-customer"
                            ? "bg-[#229799] text-white"
                            : "bg-white text-gray-500 hover:bg-gray-50"
                        }`}
                      >
                        Seller pays Customer
                      </button>
                    </div>
                  </div>
